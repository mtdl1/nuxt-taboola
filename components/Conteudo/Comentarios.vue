<template>
  <div class="bg-primary text-center py-4">
    <div class="w-11/12 sm:w-2/5 mx-auto">
      <h2 class="text-white">Comentário clientes 100PESO</h2>
      <p class="text-white font-bold mb-5">Confira a opinião de clientes que já experimentaram os <br> benefícios do 100PESO</p>
      
      <div class="flex justify-around flex-wrap">
        <nuxt-img quality="80" src="/images/content/100PESO-Comentarios-1.png" loading="lazy" alt="imagem 1" class="mb-3" />
        <nuxt-img quality="80" src="/images/content/100PESO-Comentarios-2.png" loading="lazy" alt="imagem 1" class="mb-3" />
        <nuxt-img quality="80" src="/images/content/100PESO-Comentarios-3.png" loading="lazy" alt="imagem 1" class="mb-3" />
        <nuxt-img quality="80" src="/images/content/100PESO-Comentarios-4.png" loading="lazy" alt="imagem 1" class="mb-3" />
        <nuxt-img quality="80" src="/images/content/100PESO-Comentarios-5.png" loading="lazy" alt="imagem 1" class="mb-3" />
        <nuxt-img quality="80" src="/images/content/100PESO-Comentarios-6.png" loading="lazy" alt="imagem 1" class="mb-3" />
        <!-- teste -->
      </div>

    </div>
  </div>
</template>